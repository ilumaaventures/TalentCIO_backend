...
1. **Tier A (Company Admin → Employee)**:
   - **Scope**: Same-tenant only (`req.user.companyId === target.companyId`).
   - **Permission required**: `user.impersonate` (or wildcard `*` / Admin role).
   - **Constraint**: Cannot impersonate other administrators, privileged users, or inactive accounts.
...
2. **Tier B (Platform Super Admin → Tenant User)**:
   - **Scope**: Any tenant user globally across the entire platform.
   - **Permission required**: `impersonateUsers` permission on `SuperAdminUser` (or `Super Admin` role).
   - **Constraint**: Mandatory reason/ticket reference required before starting; cannot impersonate root workspace admins or inactive accounts.
...
- **Hard 30-Minute Expiry**: Tokens are issued with a 30-minute expiration (`30m`), and their state is tracked in the `ImpersonationSession` collection.
...
- **Action Blocking**: Sensitive routes (e.g. `PUT /api/auth/change-password`, `PUT /api/admin/users/:id`, `DELETE /api/admin/users/:id`, `PATCH /api/admin/users/:id/status`, `PUT /api/admin/users/:id/role`) are guarded with `blockDuringImpersonation` middleware (`403 Forbidden`).
